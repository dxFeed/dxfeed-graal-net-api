...
{
    private readonly InstrumentProfileReaderNative ipf = InstrumentProfileReaderNative.Create();
    ...
    /// <summary>
    ///  Returns {@code true} if IPF was fully read on last <see cref="ReadFromFile(string)"/> operation.
    /// </summary>
    ...
    public List<InstrumentProfile> ReadFromFile(string address, string? user, string? password) =>
        ipf.ReadFromFile(address, user, password);
}
